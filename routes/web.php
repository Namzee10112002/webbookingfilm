...
Route::prefix('admin')->name('admin.')->middleware([CheckLoggedIn::class, CheckAdminOrStaff::class])->group(function () {
    ...
    Route::patch('movies/{movie}/toggle', [MovieController::class, 'toggle'])->name('movies.toggle');

    // Quản lý bình luận tách riêng
    Route::get('movies/{movie}/comments', [MovieController::class, 'comments'])->name('movies.comments');
    Route::patch('comments/{comment}/toggle', [MovieController::class, 'toggleComment'])->name('comments.toggle');

    Route::get('companies/order', [OrderController::class, 'chooseCompany'])->name('choose.company');
    Route::get('companies/{company}/order', [OrderController::class, 'byCompany'])->name('company');

    // chọn rạp trong 1 hãng
    Route::get('theaters/order', [OrderController::class, 'chooseTheater'])->name('choose.theater');
    Route::get('theaters/{theater}/order', [OrderController::class, 'byTheater'])->name('theater');

    // chọn phim
    Route::get('movies/order', [OrderController::class, 'chooseMovie'])->name('choose.movie');
    Route::get('movies/{movie}/order', [OrderController::class, 'byMovie'])->name('movie');
...
Route::prefix('admin/companies')->name('admin.companies.')->middleware([CheckLoggedIn::class, CheckAdminOrStaff::class])
    ->group(function () {
        // Hãng rạp
